...
$smtpAccounts = $smtpQuery->executeQuery($smtpSql, array('709-472-936', "Appengine", 1), 0, 1);
...
$subscribers = $query->executeQuery($sql, array(1, '709-472-936', 2, 0, 0), 0, 100);
...
for ($i = 0; $i < $subscribers->count() && $i < $smtpAccounts->count(); $i++) {
    try {
        ...
        if (strlen(trim($result)) == 2) {
            ...
            //$query->execute($sql, array($subscribers[$i]->getValue('mailing_list')));
            //$pm->commit();
        } else if (strpos($result, "404 not found") > 0) {
...
function sendMail($sender, $host, $subscriber) {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://fuji-apps-02.appspot.com/sydalg");
